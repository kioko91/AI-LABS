...
#position nodes to Madaraka Estate Network
...
#call BFS to return set of possible routes 
...
#color nodes 
node_col = ['teal' if not node in route_list else 'lime' for node in G.nodes()]
lime_colored_edges = list(zip(route_list,route_list[1:]))
#color edges 
#print(lime_colored_edges)
edge_col = ['teal' if not edge in lime_colored_edges else 'lime' for edge in G.edges()]
